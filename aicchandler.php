...
if (!empty($command) && ($skillsoftmisession=skillsoftmi_check_sessionid($sessionid))) {
	$skillsoftid = $skillsoftmisession->skillsoftid;
	$userid = $skillsoftmisession->userid;
	//$attempt = 1;

	
	if ($skillsoft = $DB->get_record('skillsoftmi',array('id'=>$skillsoftid))) {
