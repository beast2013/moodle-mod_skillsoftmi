...
$url = new moodle_url('/mod/skillsoftmi/olsadiag.php');
...
function getservertime() {
	...
	$location = $CFG->skillsoftmi_olsaendpoint;
...
if ($continue) {
	...
	$options = array(
		...
		try {
			...
			$client = new olsa_soapclientmi($endpoint.'?WSDL',$options);
...
if ($continue) {
	$html = '';
	$pollresponse = UTIL_PollForReportmi('0');
	if ($pollresponse->errormessage == get_string('skillsoft_olsassoapauthentication','skillsoftmi')) {
		$continue = false;
		$html .= $fail;
