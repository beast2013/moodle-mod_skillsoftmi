 ...
 */
function startsWithmi($Haystack, $Needle){
...
class aicchandlermi {
	...
	{
		$this->cmi = new cmimi($enforcestrictstudentid);
		$this->user = $user;
	...
	public function parsehacp($aiccdata)
	{
		...
		$AICCTokens = new StringTokenizermi($aiccdata, "\r\n");
		$AICCToken = "";

		while ($AICCTokens->hasMoreTokens()) {
			if(!startsWithmi($AICCToken, "[")) {
			...
			if(!$AICCToken == "")
			{
				...
				{
					if(!startsWithmi($AICCToken,"[") && (!$AICCToken ==""))
					{
	...
	public function getdata()
	{
		...
		$userdata=skillsoftmi_get_tracks($this->skillsoft->id,$this->user->id,$this->attempt);
		...
		$this->cmi->core->student_id = $CFG->skillsoftmi_accountprefix.$this->user->{$CFG->skillsoftmi_useridentifier};
	...
	public function putparam($aiccdata,$return=false)
	{
		...
		if ($CFG->skillsoftmi_trackingmode == TRACK_TO_LMSmi) {
			//Persist the data
			$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_location', $this->cmi->core->lesson_location);
			$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_status', strtolower($this->cmi->core->lesson_status));
			$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_status flag', strtolower($this->cmi->core->lesson_status_exit));
			$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]score', $this->cmi->core->score->raw);
			$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]session_time', $this->cmi->core->session_time);
			$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE_LESSON]', $this->cmi->core->core_lesson);
	...
	public function processtdr($tdr,$attempt=1)
	{
		...
		$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]time', $value);
		
		$id = skillsoftmi_setFirstAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $tdr->timestamp);
		$id = skillsoftmi_setLastAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $tdr->timestamp);
		
		if ( (substr($this->cmi->core->lesson_status,0,1) == 'c' || substr($this->cmi->core->lesson_status,0,1) == 'p'))
		{
			$id = skillsoftmi_setCompletedDate($this->user->id, $this->skillsoft->id, $this->attempt, $tdr->timestamp);
		}

		$id = skillsoftmi_setAccessCount($this->user->id, $this->skillsoft->id, $this->attempt);

		$id = skillsoftmi_setFirstScore($this->user->id, $this->skillsoft->id, $this->attempt, $this->cmi->core->score->raw);
		$id = skillsoftmi_setCurrentScore($this->user->id, $this->skillsoft->id, $this->attempt, $this->cmi->core->score->raw);
		$id = skillsoftmi_setBestScore($this->user->id, $this->skillsoft->id, $this->attempt, $this->cmi->core->score->raw);
	...
	public function exitau($return=false) {
		...
		if ($CFG->skillsoftmi_trackingmode == TRACK_TO_LMSmi) {
			...
			$now = time();
			$id = skillsoftmi_setFirstAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $now);
			$id = skillsoftmi_setLastAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $now);
	
			if ( (substr($this->cmi->core->lesson_status,0,1) == 'c' || substr($this->cmi->core->lesson_status,0,1) == 'p'))
			{
				$id = skillsoftmi_setCompletedDate($this->user->id, $this->skillsoft->id, $this->attempt, $now);
			}
		...
		} elseif($CFG->skillsoftmi_trackingmode == TRACK_TO_OLSA_AUTOMATICSYNCmi) {

			//Request results from SkillSoft
			$resp = UD_GetAssetResultsmi($this->user->username, $this->skillsoft->assetid);
			...
			if(($resp->success) && ($resp->result->RESULT->timesaccessed > 0)) {
				...
				$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_location', '');
				$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_status flag', '');
				$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]score', '');
				$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE_LESSON]', '');
				$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]session_time', $duration);
				$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]time', $duration);
				$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_status', strtolower($resp->result->RESULT->status));
				$id = skillsoftmi_setFirstAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $this->isotoepoch($resp->result->RESULT->firstaccessdate_iso));
				$id = skillsoftmi_setLastAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $this->isotoepoch($resp->result->RESULT->lastaccessdate_iso));
				$id = skillsoftmi_setAccessCount($this->user->id, $this->skillsoft->id, $this->attempt, $resp->result->RESULT->timesaccessed);	
		
				if (mb_strtoupper(substr($resp->result->RESULT->status,0,1)) == 'C' || mb_strtoupper(substr($resp->result->RESULT->status,0,1)) == 'P') {
					$id = skillsoftmi_setCompletedDate($this->user->id, $this->skillsoft->id, $this->attempt,$this->isotoepoch($resp->result->RESULT->completiondate_iso));
					$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_status', 'Completed');		
				}
		
				$id = skillsoftmi_setFirstScore($this->user->id, $this->skillsoft->id, $this->attempt, $resp->result->RESULT->overallcurrentscore);
				$id = skillsoftmi_setCurrentScore($this->user->id, $this->skillsoft->id, $this->attempt, $resp->result->RESULT->overallcurrentscore);
				$id = skillsoftmi_setBestScore($this->user->id, $this->skillsoft->id, $this->attempt, $resp->result->RESULT->overallhighscore);
			}
		}
		$response = '';
		$response .= 'error=0'."\r\n";
	...
	public function processreportresults($reportresults, $attempt=1)
	{
		...
		$id = skillsoftmi_setFirstAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $reportresults->firstaccessdate);
		$id = skillsoftmi_setLastAccessDate($this->user->id, $this->skillsoft->id, $this->attempt, $reportresults->lastaccessdate);

		if ( (substr($this->cmi->core->lesson_status,0,1) == 'c' || substr($this->cmi->core->lesson_status,0,1) == 'p')) {
			$id = skillsoftmi_setCompletedDate($this->user->id, $this->skillsoft->id, $this->attempt, $reportresults->completeddate);
		}

		$id = skillsoftmi_setAccessCount($this->user->id, $this->skillsoft->id, $this->attempt,$reportresults->accesscount);
		$id = skillsoftmi_setFirstScore($this->user->id, $this->skillsoft->id, $this->attempt, $reportresults->firstscore);
		$id = skillsoftmi_setCurrentScore($this->user->id, $this->skillsoft->id, $this->attempt, $reportresults->currentscore);
		$id = skillsoftmi_setBestScore($this->user->id, $this->skillsoft->id, $this->attempt, $reportresults->bestscore);
		
		//Need to do these last to ensure grades correctly entered
		//Persist the data
		$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]lesson_status', $this->cmi->core->lesson_status);
		$id = skillsoftmi_insert_track($this->user->id, $this->skillsoft->id, $this->attempt, '[CORE]score', $this->cmi->core->score->raw);
