...
class olsa_soapclientmi extends SoapClient{
	 ...
	 */
	private function downloadfile($url, $filename , $forcedownload=false, $cachefolder='skillsoftmi' , $cachetime=86400) {
		global $CFG;
		...
		if (!make_cache_directory($folder, false)) {
			...
			throw new Exception('Could not create WSDL Cache Folder (skillsoftmi): '.$basefolder.$folder);
		}
	...
	private function retrieveWSDL($wsdl) {
		...
		if ($CFG->skillsoftmi_clearwsdlcache == 1) {
			$forcedownload = true;
		} else {
			$forcedownload = false;
		}
		...
		if ($wsdlcontent = $this->downloadfile($wsdl,'olsa.wsdl',$forcedownload)) {
			...
			if ($wsdlcontent->status != 200) {
				...
				$this->processExternalSchema($wsdlcontent->filepath, $CFG->skillsoftmi_olsaendpoint.'/../', $forcedownload);
			}
		}
		//If we forced a redownload clear the setting
		if ($CFG->skillsoftmi_clearwsdlcache == 1) {
			set_config('skillsoftmi_clearwsdlcache', 0);
		}
 ...
 */
class olsaresponsemi implements IteratorAggregate {
 ...
 * @return string
 */
function olsadatatohtmlmi($text) {
 ...
 * @return book
 */
function isolsaconfigurationsetmi() {
	global $CFG;
	if (!isset($CFG->skillsoftmi_olsaendpoint, $CFG->skillsoftmi_olsacustomerid, $CFG->skillsoftmi_olsasharedsecret)) {
		return false;
	} else {
		//They are set BUT are they empty
		if (empty($CFG->skillsoftmi_olsaendpoint) || empty($CFG->skillsoftmi_olsacustomerid) || empty($CFG->skillsoftmi_olsasharedsecret)) {
			return false;
		}
	}
	return true;
 ...
 */
function AI_GetXmlAssetMetaDatami($assetid) {
	global $CFG;

	if (!isolsaconfigurationsetmi()) {
		$response = new olsaresponsemi(false,get_string('skillsoft_olsasettingsmissing','skillsoftmi'),NULL);
	} else {

		//Set local OLSA Variables
		$endpoint = $CFG->skillsoftmi_olsaendpoint;
		$customerId = $CFG->skillsoftmi_olsacustomerid;
		$sharedsecret = $CFG->skillsoftmi_olsasharedsecret;
		...
		$options = array(
			...
			$client = new olsa_soapclientmi($wsdlurl,$options);
			...
			if (is_soap_fault($result)) {
				if (stripos($result->getmessage(),'security token could not be authenticated or authorized')) {
					...
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapauthentication','skillsoftmi'),NULL);
				} elseif (stripos($result->getmessage(), 'does not exist.')){
					//Asset ID is invalid
					//print_error('olsassoapinvalidassetid','skillsoft','',$id);
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapinvalidassetid','skillsoftmi',$assetid),NULL);
				} else {
					//General SOAP Fault
					//print_error('olsassoapfault','skillsoft','',$result->getmessage());
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapfault','skillsoftmi',$result->getmessage()),NULL);
				}
			} else {
				$asset = $result->metadata->asset;
				$response = new olsaresponsemi(true,'',$asset);
			}
	}
	return $response;
}
 ...
 * @return olsasoapresponse olsasoapresponse->result is an object representing the deserialised XML response
 */
function UD_GetAssetResultsmi($userid,$assetid,$summarylevel=true) {
	global $CFG;
		...
		$options = array(
			...
			if (is_soap_fault($result)) {
				...
				if (!stripos($result->getmessage(),'security token could not be authenticated or authorized') == false) {
					...
					//User ID is invalid
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapinvaliduserid','skillsoftmi',$userid),NULL);
				} elseif (!stripos($result->getmessage(), 'are no results for') == false){
					//No results repond as OK with NULL object
					$response = new olsaresponsemi(true,'',NULL);
				} else {
					//General SOAP Fault
					//print_error('olsassoapfault','skillsoft','',$result->getmessage());
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapfault','skillsoftmi',$result->getmessage()),NULL);
				...
				$response = new olsaresponsemi(true,'',$results);
			}
	}
	return $response;
}
 ...
 *
 * @return olsasoapresponsemi olsasoapresponsemi->result is a NULL object
 */
function OC_InitializeTrackingDatami() {
	global $CFG;
 ...
 */
function OC_AcknowledgeTrackingDatami($handle) {
	global $CFG;
		...
		$options = array(
			...
			if (is_soap_fault($result)) {
				...
				if (!stripos($result->getmessage(),'security token could not be authenticated or authorized') == false) {
					...
					$response = new olsaresponsemi(false,get_string('skillsoft_odcnoresultsavailable','skillsoftmi'),NULL);
				} else {
					//General SOAP Fault
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapfault','skillsoftmi',$result->getmessage()),NULL);
				}
			} else {
				$response = new olsaresponsemi(true,'',$result);
			}
	}
	return $response;
}
 ...
 */
function SO_GetMultiActionSignOnUrlmi(
$userName,
$firstName = '',
$lastName = '',
$email = '',
 ...
 */
function UD_InitiateCustomReportByUserGroupsmi($group,$startDate='',$endDate='',$dateToUse='any',$listBy='user',$includeSubgroups=true,$includeDeactivatedUsers=true,$reportFormat='CSV') {
	global $CFG;
 ...
 * @return olsasoapresponse olsasoapresponse->result. result->handle is the report handle used for polling
 */
function UD_InitiateCustomReportByUsersmi($username='',$startDate='',$endDate='',$dateToUse='any',$listBy='user',$includeDeactivatedUsers=true,$reportFormat='CSV') {
	global $CFG;
 ...
 * @return olsasoapresponse olsasoapresponse->result is a NULL object
 */
function UTIL_PollForReportmi($handle) {
	global $CFG;
		...
		$options = array(
			...
			if (is_soap_fault($result)) {
				...
				if (!stripos($result->getmessage(),'security token could not be authenticated or authorized') == false) {
					...
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapreportnotready','skillsoftmi'),NULL);
				} elseif (!stripos($result->detail->exceptionName, 'ReportDoesNotExistFault') == false){
					//Report not ready yet
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapreportnotvalid','skillsoftmi',$handle),NULL);
				} else {
					//General SOAP Fault
					//print_error('olsassoapfault','skillsoftmi','',$result->getmessage());
					$response = new olsaresponsemi(false,get_string('skillsoft_olsassoapfault','skillsoftmi',$result->getmessage()),NULL);
				}
			} else {
				$response = new olsaresponsemi(true,'',$result);
